...
class TaskController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->filled('prioriteit')) {
            $query->where('tasks.prioriteit', $request->prioriteit);
        }
        ...
        if ($request->filled('deadline_van')) {
            $query->where('tasks.deadline', '>=', $request->deadline_van);
        }
        if ($request->filled('deadline_tot')) {
            $query->where('tasks.deadline', '<=', $request->deadline_tot);
        }
    ...
    {
        return DB::transaction(fn () => $this->storeAssignedTask($request));
    }

    private function storeAssignedTask(Request $request)
    {
        ...
        app(AssignmentNotifications::class)->sync($task, $request, 'task');
    ...
    {
        return DB::transaction(fn () => $this->updateAssignedTask($request, $id));
    }

    private function updateAssignedTask(Request $request, string $id)
    {
